#20250208
#python3 test_pkg_python/test_pkg_python/DBSCAN.py 
#rキーが押されたら出入り口検出
...
class ee_detection(Node):
    def __init__(self):
        ...
        self.key_thread = threading.Thread(target=self.read_key)
    ...
    def scan_callback(self, msg):
        ...
        #self.get_logger().info(f'Received scan data: {len(msg.ranges)} points')
    ...
    def read_key(self):
        ...
        while True:
            ...
            if key == 'r':
                self.get_logger().info('read /scan data')
    ...
    '''
    def readcsv(self):
        filename='test_pkg_python/data/test.csv'
        with open(filename, encoding='utf8',newline='') as f:
            csvreader = csv.reader(f)
            next(csvreader)
            self.data = [row for row in csvreader]
        np_left = np.array(self.data,dtype=float)
        self.xy_left = left_data(np_left)
        self.xy = self.xy_left
        #self.plot_data_Nx2_red(self.xy_left)
    ...
    def analyze(self):
        ...
        self.xyID = np.column_stack((self.xy[:,0], self.xy[:,1], self.labels))
        self.split_data()
        self.get_logger().info(f'dbscan finished')
    ...
    def split_data(self):
        ...
        for i in range(len(combination_list)):#i:組み合わせ数だけ繰り返す
            ...
            if len(now_list) > 5 and len(next_list) > 5:
...
def left_data(data):
    ...
    for i in range(len(filter_data)):
        if 360 < data[i,0]:
            ...
            y=-int(filter_data[i][1]) * math.sin((315-int(filter_data[i][0])/4)*p/180)-0
            list_x.append(x)
            list_y.append(y)
...
def main():
    ...
    try:
        ...
        pass
    plt.scatter(node.xy[:,1],node.xy[:,0],c=node.labels,s=10)
    plt.plot([node.door_data[1],node.door_data[3]],[node.door_data[0],node.door_data[2]],color = 'red')
    print(node.cluster_num)
    plt.axvline(x=0, color='black')
    plt.axhline(y=0, color='black')
    plt.xlim(-node.lim, node.lim) # x軸の範囲
    plt.ylim(-node.lim, node.lim) # y軸の範囲
    plt.savefig('plot2.png')
    node.destroy_node()
    rclpy.shutdown()
    #lider.readcsv()
    #lider.split_data()
    #plt.scatter(lider.xy[:,1],lider.xy[:,0],c=lider.labels,s=10)
    #plt.plot([lider.door_data[1],lider.door_data[3]],[lider.door_data[0],lider.door_data[2]],color = 'red')
    #print(lider.door_data)
    #print(lider.cluster_num)
    #plt.axvline(x=0, color='black')
    #plt.axhline(y=0, color='black')
    #plt.savefig('test_pkg_python/plot.png')
