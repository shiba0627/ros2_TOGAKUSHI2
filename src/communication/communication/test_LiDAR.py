...
class LiDAR_Subscriber(Node):
    ...
    def __init__(self):
        ...
        self.lower_than_floor_info = BoolMultiArray()
        self.combined_info = BoolMultiArray()
    ...
    def callback(self, lidar_msg):
        ...
        self.obstacle_info.data = [False]*10 # 障害物情報（どこにあるか）のリスト
        self.lower_than_floor_info.data = [True]*10 # 床より低いデータ情報（どこにあるか）のリスト
    ...
    def Filter_Points(self, xyz_points):
        ...
        self.data_lower_than_floor_list = [] # 床より低いデータを格納するリスト
        x = []
        y = []
        z = []
        for data in xyz_points:
            if -3.0 <= data[2] <= 0.0: # nan以外のデータを取り出す
                x = data[0]*1000  
                y = data[1]*1000
                ...
                if distance <= 100000:
                    if (-54 < direction < 54) and (1100 <= x): # ユーザ、PCを除く前方の距離データは専用のリストに格納、エッジ効果による外れ値除去のため
                        ...
                        if 20 < z: 
                            ...
                            self.data_lower_than_floor_list.append((x, y, z, intensity, distance, direction))
    ...
    def Obstacle_Detection(self):
        if self.detection_range.data == 0.0:
            ...
            '''床以下の監視'''
            if self.data_lower_than_floor_list:
                cw_lower_list = []
                for x, y, z, intensity, distance, direction in self.data_lower_than_floor_list:
                    if (-74 <= direction < -54): #cw旋回方向に検出
                        print(x,y,z)
                        if (0 < z) and (distance <= 900):
                            self.lower_than_floor_info.data[3] = False # 床として検出
                        elif z < 0:
                            cw_lower_list.append((distance, z))

                cw_std_dev = self.calculate_std_dev(cw_lower_list) #cw旋回方向の標準偏差算出

                if cw_std_dev is None:
                    #self.get_logger().info('検知範囲内には下り階段/段差/スロープなし')
                    self.lower_than_floor_info.data[3] = False # 床として検出

                elif cw_std_dev < 4.0: # zのばらつきが小さい場合
                    self.lower_than_floor_info.data[3] = True # 下り段差として検出

                else:
                    cw_angle = self.angle_calculation_2(cw_lower_list) #cw旋回方向の傾斜角度算出
                    if 8.0 <= cw_angle:
                        self.lower_than_floor_info.data[3] = False # 下りスロープとして検出
                    else:
                        self.lower_than_floor_info.data[3] = True # 下り階段として検出

            self.combined_info.data = [a or b for a, b in zip(self.obstacle_info.data, self.lower_than_floor_info.data)]
            #print(self.combined_info)
            

            global mode_flag
            if mode_flag : # ユーザ操縦モードの場合、障害物情報をそのまま配信
                self.pub.publish(self.combined_info)
            else:          # 介助者操縦モードの場合、周囲に障害物がないことにして配信（衝突防止動作を無効に）
                self.combined_info.data = [False]*10
                self.pub.publish(self.combined_info)

    '''標準偏差の算出'''
    def calculate_std_dev(self, list_data):
        # distanceが900以下のデータがある（下り階段/段差/スロープが検知範囲内）か確認
        has_valid_data = any(row[0] <= 900 for row in list_data)
        ...
        # 元のデータの2列目を抽出
        z_list = [row[1] for row in list_data]
        
        # 標準偏差を計算
        std_dev = np.std(z_list)
    ...
    '''傾斜角度の算出（床より下のデータ）'''
    def angle_calculation_2(self, list_data):
        '''
        # distanceが900以下のデータがある（下り階段/スロープが検知範囲内）か確認
        has_valid_data = any(row[0] <= 900 for row in list_data)
        ...
        for data in list_data:
            angle = math.degrees(math.atan2(data[1], data[0]))
            angle_list.append(angle)
        
        # 平均を計算し返す
        angle_ave = sum(angle_list) / len(angle_list)
        return abs(angle_ave)
